...
def evaluate_dataset(model, dataset, fold,shuffle,name, save=False):
    ...
    with torch.no_grad():
        for datas, label,(smiles1,smiles2) in loader:
    ...
    if save:
        df = pd.DataFrame({'Experimental Value': y, 'Predicted Value': pred, 'SMILES1': smi1, 'SMILES2': smi2})
...
def cross_validation(dataset,config,df,num_folds):
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(indices)):
        ...
        if config['Load pretrained model']== True:
            ...
            state_dict = torch.load("./pretrained_model_5M.pth", map_location=device)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("./data/data_cleaned_vis.csv",encoding='unicode_escape')
    ...
    print(normalizer.max, normalizer.min, labels.shape)
