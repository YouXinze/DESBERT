...
def evaluate_dataset(model, dataset, fold,shuffle,name, save=False):
    ...
    y, pred,smi1,smi2,mol1,mol2 = [],[],[],[],[],[]


    with torch.no_grad():
        for datas, label,(smiles1,smiles2,molar1,molar2) in loader:
    ...
    if save:
        df = pd.DataFrame({'Experimental Value': y, 'Predicted Value': pred, 'SMILES1': smi1, 'SMILES2': smi2, 'molar1': mol1})
